...
export const initPage = ({commit}, {}) => {
    ...
    wx.getLoginCode().then((loginCode) => {
    	...
    	api.getOpenId({code: loginCode}).then((res) => {
    		commit(types.SAVE_OPENID, {openId: res.data.openid})
    	})
    })
    ...
    api.getBannerList().then((res) => {
        ...
        commit(types.SAVE_BANNER_ACTIVATE, {banners, activates})
    })
}

export const requsetProductDetail = ({commit}, {productId}) => {
    let params = {serial_number: productId, openid: store.state.openId};
    api.getProductDetail(params).then((res) => {
        commit(types.SAVE_PRODUCT_DETAIL, {productDetail: res.data})
    })
}